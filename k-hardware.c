...
static void set_gate(x86_64_gatedescriptor *gate, uint64_t type, int dpl,
                     uintptr_t function) {
  gate->gd_low = (function & 0x000000000000FFFFUL) | (SEGSEL_KERN_CODE << 16) |
                 type | ((uint64_t)dpl << 45) | X86SEG_P |
...
void segments_init(void) {
  ...
  asm volatile("lgdt %0\n\t"
               ...
               "lidt %2"
               :
               : "m"(gdt), "r"((uint16_t)SEGSEL_TASKSTATE), "m"(idt)
               : "memory");
...
static x86_64_pagetable *
lookup_l4pagetable(x86_64_pagetable *pagetable, uintptr_t va, int perm,
                   x86_64_pagetable *(*allocator)(void));

int virtual_memory_map(x86_64_pagetable *pagetable, uintptr_t va, uintptr_t pa,
                       size_t sz, int perm,
                       x86_64_pagetable *(*allocator)(void)) {
...
static void parallel_port_putc(printer *p, unsigned char c, int color) {
  ...
  outb(IO_PARALLEL1_CONTROL, IO_PARALLEL_CONTROL_SELECT |
                                 ...
                                 IO_PARALLEL_CONTROL_STROBE);
  outb(IO_PARALLEL1_CONTROL,
       IO_PARALLEL_CONTROL_SELECT | IO_PARALLEL_CONTROL_INIT);
}
